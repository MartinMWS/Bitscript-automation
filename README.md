...
Some scripts are meant to be manually run as needed. Most scripts take arguments to tweak or customize their behaviour based on your preferences or special circumstance. More info [below](#customizing-script-behaviour).
...
For more and up to date information read the offical Bitburner documentation.
...
You may find it useful to set up one or more aliases with the default options you like rather than editing the file itself. (Pro-tip, aliases support tab-auto-completion). Here are some examples:
...
The file should have the name `some-script-name.js.config.txt`

Your config file should be either of the following two formats
